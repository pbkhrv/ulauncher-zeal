...
class ZealExtension(Extension):
    ...
    def process_docset_kw_arg_query(self, ukw, kw, arg):
        ...
        if kw:
            docsets = self.list_matching_docsets(kw)
        else:
            docsets = [
        ...
        if len(docsets) > MAX_DOCSETS_VISIBLE:
            ...
            tail.append(
                ExtensionSmallResultItem(
                    ...
                    name="...{} more docset{} available, refine the query to filter...".format(
                        more, sss
                    ),
    ...
    def list_matching_docsets(self, kw):
        kws = zeal.fuzzy_filter_keywords(self.active_kws, kw)
        dcs = []
    ...
    def reload_docsets(self):
        dcs = zeal.list_installed_docsets(self.get_docsets_path())
        self.cached_docsets = dict((d["title"], d) for d in dcs)
        self.kw_docset_map = dict()
        for d in dcs:
            for kw in d["keywords"]:
                self.kw_docset_map[kw] = d["title"]
...
class KeywordQueryEventListener(EventListener):
    ...
    def on_event(self, event, extension):
        ...
        if arg:
            ...
            return extension.process_docset_kw_arg_query(ukw, kw, arg)
        else:
...
class ItemEnterEventListener(EventListener):
    ...
    def on_event(self, event, extension):
        (kw, arg) = event.get_data()
        zeal.query_docset(kw, arg)
